...
import io.github.cboudereau.dataseries.UnionResult.Both;
import io.github.cboudereau.dataseries.UnionResult.RightOnly;
...
public class SamplesTest {
    ...
    public void intersection() {
        ...
        var actual = Series.union(s1, s2, SamplesTest::toTuple).stream().filter(x -> x.data().isPresent()).map(x -> Series.datapoint(x.point(), x.data().get())).toArray();
